...
public class FlatFileItemReaderBinary<T> extends AbstractItemCountingItemStreamItemReader<T> implements
	...
	private static final Log logger = LogFactory.getLog(FlatFileItemReaderBinary.class);
	...
	public List<String> listLine = new ArrayList<String>();
	private BufferedReaderFactory bufferedReaderFactory = new DefaultBufferedReaderFactory();

	public FlatFileItemReaderBinary() {
		setName(ClassUtils.getShortName(FlatFileItemReaderBinary.class));
	}

	/**
	 * set every one record lengths 
	 */
	...
	private String readLine() {
		...
		if (len == 0)
		{
			throw new ReaderNotOpenException("Reader must be set ervey record length.");
		}
		...
		try {
			...
			int cr = this.reader.read(cbuf, 0, len);
			if(cr != -1) {
				line= new String(cbuf, 0, cr) ;
			}else {
				line = null;
			}
